...
cases = scripts[21]
```
En las siguientes líneas convertimos el contenido de cases a formato string para poder utilizar la herramienta .find y buscar las palabras 'coronavirus-cases-daily' dentro del elemento .chart y definimos daily_cases como el contenido de cases_str a partir del elemento index
```
...
```
De igual forma que lo anterior, generamos el vector casesString con un slip seccionado donde definimos el inicio como data_start_indes y el final como data_end_index, todo a partir de la data de daily_cases
```
...
print(Date)
```
Para presentar el contenido de casesString primero sustituimos los elementos 'null' y los cambiamos por el valor numerico 0 y al igual que el paso anterior generamos una evaluacion y posteriormente imprimimos el resultado
```
